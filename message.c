...
{
	unsigned char c[4];
	unsigned char *bufffer = peer->out_msg + peer->msg_len;
	int len = MSG_SIZE - peer->msg_len;
...
int is_complete_message(unsigned char *buff,unsigned int len,int *ok_len)
{
	...
	*ok_len = 0;
	btkeyword[0] = 19;
	memcpy(&btkeyword[1], "BitTorrent protocal", 19);
	...
	for(i=0; i<len; )
	{
		...
		else if(i+5 <=len && memcmp(&buffer[i],uninterested,5)==0) i+=5;
...
int process_handshake_msg(Peer *peer,unsigned char *buff, int len)
{
	...
	if(memcmp(info_hash,buffer+28,20)!=0)
	{
		peer->state = CLOSING;
...
int process_request_msg(Peer *peer, unsigned char *buff, int len)
{
	...
	if(peer->am_choking==0 && peer->peer_interested==1)
	{
		...
		printf("***add a request FROM IP:%s index:%-d begin:%-6x ***\n", peer->ip,index,begin);
	}
	peer->start_timestamp = time(NULL);
	return 0;
}
...
int process_piece_msg(Peer *peer, unsigned char *buff, int len)
{
	...
	if(peer->peer_choking == 0)
	{
		...
		while(p != NULL)
		{
			if(p->index==index && p->begin && p->length==length)
				break;
			p = p->next;
		}
		if(p == NULL)
...
int parse_response(Peer *peer)
{
	...
	for(index = 0; index < len; )
	{
		...
		{
			process_keep_alive_msg(peer,buff+index,5);
			index += 4;
		}
		...
		{
			process_interested_msg(peer,buff+index,9);
			index += 9;
		}
